...
class BiQuGeListContentAdapter extends BaseRecyclerAdapter<BiQuGeListContentsModel> {

    BiQuGeListContentAdapter(List<BiQuGeListContentsModel> mDatas) {
        super(mDatas);
    }

    @Override
    protected int getLayoutId() {
        return R.layout.item_biquge_list_contents;
    }

    @Override
    protected void onBind(SuperViewHolder viewHolder, int position, BiQuGeListContentsModel mDatas) {
        if (position < 10) {
